...
import { useUserCreatedLists, useUser } from '@/hooks/queries/users';
...
export default function UserListsScreen() {
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
  } = useUserCreatedLists(user?.username);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <AnimatedHeader
        title={t("profile.listsScreen.title", { username: user?.displayName })}
        scrollY={scrollY}
        onBack={handleBack}
          ...
          ListHeaderComponent={
            <View style={styles.header} onLayout={(e) => setTitleY(e.nativeEvent.layout.y)}>
              <Text
                ...
                numberOfLines={1}
              >
                {t("profile.listsScreen.title", { username: user?.displayName })}
              </Text>
            </View>
          }
